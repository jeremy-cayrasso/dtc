...
////////////////////////////////////////////////////
function drawAdminTools_GameInstaller($domain,$adm_path){
	global $adm_login;
	global $adm_pass;
	global $edit_domain;
	global $addrlink;
	...
	$txt = "";
	$dir = $dtcshared_path."/game-installer";
	...
	if(isset($_REQUEST["action"]) && ($_REQUEST["action"] == "do_install" || $_REQUEST["action"] == "prepareinstall")){
		...
		if(!file_exists($dtc_pkg_info)){
			die("Game $dtc_pkg_info not found line ".__LINE__." file ".__FILE__);
		}
	...
	if(isset($_REQUEST["action"]) && $_REQUEST["action"] == "do_install"){
		...
		if($pkg_info["unpack_type"] == "tar.gz"){
			...
			die("Game methode not supported yet");
		}
...
// https://dtc.gpl-host.com/dtc/index.php?adm_login=zigo&adm_pass=toto&addrlink=gpl-host.com/package-installer&action=prepareinstall&pkg=phpbb
		...
		if($pkg_info["can_select_directory"] == "no"){
...
//		echo "</pre>";
		...
		if($pkg_info["can_select_directory"] == "yes"){
			$txt .= "<h3>Enter the directory where you want to install this game:</h3><br>";
	...
	$txt = "<h3>". _("Choose a game to install") .":</h3>";

	$txt .= "<table cellspacing=\"0\" cellpadding=\"4\" border=\"1\">";
	$txt .= "<tr><td>". _("Game name") ."</td><td>". _("Description") ."</td><td>". _("Game version") ."</td><td>". _("Need a database") ."</td><td>". _("Unpack size") ."</td><td>". _("Install") ."</td></tr>";
	if (is_dir($dir)) {
		if ($dh = opendir($dir)) {
